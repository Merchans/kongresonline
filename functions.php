...
function chi_my_load_more_scripts()
{
	...
	wp_register_script(
		'my_loadmore',
		get_stylesheet_directory_uri() . '/js/myloadmore.js',
		array('jquery')
	);
	...
	wp_localize_script('my_loadmore', 'chi_loadmore_params', array(
...
function chi_loadmore_ajax_handler()
{
	$posts = array("post_type" => "post", "posts_per_page" => 2 , "post_status" => 'publish', 'orderby' => 'post_date',  'order' => 'DESC');
	...
	if ($query->have_posts()) {
		...
		while ($query->have_posts()) {
			...
			$first_two_videos[] = get_the_ID();
		}
	}
	wp_reset_query();

	$posts = array("post_type" => "post", "posts_per_page" => get_option("posts_per_page"), "post_status" => 'publish', 'orderby' => 'post_date',  'order' => 'DESC', 'post__not_in' => $first_two_videos);
	...
	$args['post__not_in'] = array_merge($address_post_id, $first_two_videos);
	...
	if (have_posts()) :
		...
		while (have_posts()) : the_post(); ?>
			<?php if (get_post_type() == "post" || get_post_type() == "chi_video") : ?>
...
add_action('wp_enqueue_scripts', 'chi_my_load_more_scripts');
add_action('wp_ajax_loadmore', 'chi_loadmore_ajax_handler'); // wp_ajax_{action}
add_action('wp_ajax_nopriv_loadmore', 'chi_loadmore_ajax_handler');
