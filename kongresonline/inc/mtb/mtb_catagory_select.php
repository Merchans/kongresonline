...
function chi_selected_one_option_for_claim($post)
{
    ...
    $args                  = array('post_type' => array('post', 'chi_video'), 'numberposts' => -1, 'category' => "$post->term_id");
    ...
    <tr class="form-field term-parent-wrap">
        ...
        <td>
            <select name="chi_selected_one_options" id="chi_selected_one_options" class="postform">
                    <option class="level-0" value="1" <?php if (get_term_meta($post->term_id, "_chi_selected_one_options")[0] == 1){  echo "selected"; }?>>Hlavní okno video, sekundární okna články</option>
                    <option class="level-0" value="2" <?php if (get_term_meta($post->term_id, "_chi_selected_one_options")[0] == 2){  echo "selected"; }?>>Vždy nejnovější obsah</option>
                    <option class="level-0 show-claim-custom" value="3" <?php if (get_term_meta($post->term_id, "_chi_selected_one_options")[0] == 3){  echo "selected";}?>>Vlastní výběr</option>
             </select>
            <p class="description">Pokud chcete, v této sekci můžete změnit základní chování úvodních oken v speciálu.</p>
        </td>
    </tr>
    <tr id="chi_claim_posts">
        <th scope="row"><label for="chi_selected_in_claim_posts">Vyberte obsah kterých chcete zobrazovat na titulní straně speciálu</label></th>
        <td>
            <select name="chi_selected_in_claim_posts[]" id="chi_selected_in_claim_posts" multiple="multiple" class="claim_articles" style="width: 100%;">
                <?php
                foreach ($chi_posts_loop as $chi_post) {
                    $dir_id = $chi_post->ID;

                    $selected = (in_array($dir_id, $chi_selected_in_claim_posts_values)) ? 'selected="selected"' : '';  $print = true;
                    ?>
